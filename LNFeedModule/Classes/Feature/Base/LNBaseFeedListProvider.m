...
- (LNListJSONResponse *)feedCellLayoutResponseWithFeedResponse:(LNListJSONResponse *)feedListResponse
{
    ...
    for (LNFeed *feed in feedListResponse.dataList) {
        LNFeedCellLayout *feedLayout = [[LNFeedCellLayout alloc] init];
        feedLayout.feed = feed;
        [feeds addObject:feedLayout];
    }
    feedListResponse.dataList = [feeds copy];
...
- (NSDictionary *)localData
{
    ...
    for (NSInteger index = 0; index < self.pageSize; index++) {
        ...
        feed[@"creator"] = [user copy];
        feed[@"relatedUsers"] = @[user];
        NSMutableArray *topics = [[NSMutableArray alloc] init];
        for (NSInteger i = 0; i < index%5; i++) {
            NSMutableDictionary *topic = [[NSMutableDictionary alloc] init];
            NSString *topicId = [NSString stringWithFormat:@"%@-%@", @(index),@(i)];
            topic[@"topicId"] = topicId;
            topic[@"name"] = [NSString stringWithFormat:@"这是一个很好的话题%@", topicId];
            [topics addObject:[topic copy]];
        }
        ...
        for (NSInteger i = 0; i < index; i++) {
            ...
            [comments addObject:[comment copy]];
        }
        ...
        if (index%2 == 0) {
            ...
            feed[@"originFeed"] = [feed copy];
        }
        feed[@"createTime"] = @([[NSDate date] timeIntervalSince1970]);
        [dataList addObject:[feed copy]];
        
    }
    
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    dict[@"data"] = [dataList copy];
    dict[@"code"] = @(200);
    dict[@"message"] = @"请求成功";
    return [dict copy];
}
