...
@interface LNFeedRecommendListProvider ()<LNHTTPRequestDelegate>
...
- (void)requestWithCompletion:(LNListRequestCompletion)completion
{
    ...
    } paramsCreator:^(NSMutableDictionary * _Nonnull params) {
...
//        while (dataList.count >= (lastLocation + interval)) {
//
//            NSArray *operationArr = [dataList subarrayWithRange:NSMakeRange(lastLocation, interval)];
//            NSArray *cellLayouts = [self createFeedCellLayoutsWithFeeds:operationArr];
//            if (lastLocation == 0) {
//                if (completion) {
//                    completion(cellLayouts, nil);
//                }
//            }else{
//                [self handleLoadingMoreData:cellLayouts error:nil];
//            }
//
//
//            lastLocation = interval + lastLocation -1;
//        }
//        if (dataList.count > lastLocation && dataList.count < (lastLocation + interval)) {
//            NSArray *operationArr = [dataList subarrayWithRange:NSMakeRange(lastLocation, dataList.count - lastLocation)];
//            NSArray *cellLayouts = [self createFeedCellLayoutsWithFeeds:operationArr];
//            if (lastLocation == 0) {
//                if (completion) {
//                    completion(cellLayouts, nil);
...
- (void)requestFailure:(nonnull LNHTTPRequest *)request error:(nonnull NSError *)error {
    
}

- (void)requestSucceed:(nonnull LNHTTPRequest *)request data:(nonnull id)data {
